...
export default async function handler(req, res) {
    ...
    try {
        ...
        // 2. Cleanup Analytics (Visits before today)
        ...
        const endLast = new Date(today.getFullYear(), today.getMonth(), 1).getTime();
        ...
        const anaKeys = Object.entries(anaData).filter(([_, v]) => !v.timestamp || v.timestamp < today.getTime()).map(([k]) => k);
        if (anaKeys.length > 0) {
