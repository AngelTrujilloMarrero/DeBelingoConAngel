...
const APP_NAME = 'cleanup_messages_app';
...
export default async function handler(req, res) {
    try {
        ...
        const messagesRef = db.ref('guestbook/messages');
        ...
        const keysToDelete = Object.entries(data)
            .filter(([_, value]) => value.timestamp && (now - value.timestamp) > TWO_MONTHS_MS)
            .map(([key]) => key);
        ...
        console.error('[Cleanup Messages Error]:', error.message);
